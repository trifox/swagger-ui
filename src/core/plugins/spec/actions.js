...
const debResolveSubtrees = debounce(async () => {
  ...
  console.log('debResolveSubtrees system ', system)
  if (!system) {
  ...
  try {
    var batchResult = await requestBatch.reduce(async (prev, path) => {
      ...
      console.log('Reduce inner prev/path', {
        baseDoc: proxyUrl,
        modelPropertyMacro,
        parameterMacro,
        requestInterceptor,
...
export const requestResolvedSubtree = path => system => {
  console.log('Request resolve subtree called ', path, system)
...
export const execute = ({path, method, ...extras} = {}) => (system) => {
  console.log('Execute called', path, method, ...extras)
